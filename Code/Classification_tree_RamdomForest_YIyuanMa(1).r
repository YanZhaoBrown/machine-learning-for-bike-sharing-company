...
dayset <- read.csv('dayset2Season.csv')
#View(dayset)
...
# Set the random seed
set.seed(1)
dayset.train$cluster <- as.factor(dayset.train$cluster)

cnt_Forest = randomForest(cluster ~., data=dayset,mtry=sqrt(32),ntree=5000)
#summary(myForest)
cnt_Forest
...
registered_Forest = randomForest(cluster ~.,  data=dayset, mtry=sqrt(32),ntree=5000)
#summary(myForest)
registered_Forest
...
Cluster=casualCluster$cluster-1
Cluster
cluster = ifelse(Cluster < 1, "Less", "More")
lengths(cluster)
#plot(cnt, col=(1+cntCluster$cluster), main="K-Means Clustering Results with K=2", xlab="instant", ylab="cnt", pch=6, cex=2)
dayset = data.frame(dayset,cluster)
#View(dayset)
...
casual_Forest = randomForest(cluster ~., data=dayset ,mtry=sqrt(32), ntree=5000)
#summary(myForest)
casual_Forest
