...
cnt_alldayset = dayset[c(1:27,32:34,36:39,42)]
cnt_alldayset.train = dayset.train[c(1:27,32:34,36:39,42)]
cnt_alldayset.test = dayset.test[c(1:27,32:34,36:39,42)]
...
### 
{
  all_prunetree_results = data.frame(treesize=integer(0), train.error=numeric(0), test.error=numeric(0))
  ...
  for (treesize in 2:10)
  {
    ...
    #png(paste("all_PruneTree_", treesize, ".png", sep=""))
    #plot(dayset$instant, cnt_alldayset$cnt, xlab="Time (Days)", ylab="Bikeshare Total Users")
    #points(dayset.train$instant, cntp_alldayset_Prune_Tree.train, col="blue")
   # points(dayset.test$instant, cntp_alldayset_Prune_Tree, col="red", lwd=3)
   ...
   ## mtext(paste("tree.size:",treesize , sep = ""),side = 3,line = 2,adj = 1, col="blue")
    ...
    all_prunetree_results = rbind(all_prunetree_results, cbind(treesize, train.error, test.error))
  }
}

plot(x=all_prunetree_results$treesize,y=all_prunetree_results$train.error,pch = 8,col='blue')
lines(x=all_prunetree_results$treesize,y=all_prunetree_results$train.error,col='blue')
points(x=all_prunetree_results$treesize,y=all_prunetree_results$test.error,pch = 8,col='red')
lines(x=all_prunetree_results$treesize,y=all_prunetree_results$test.error,pch = 8,col='red')
...
cnt_alldayset_CVForest_result = randomForest(cnt~., data=cnt_alldayset, newdata=cnt_alldayset.train,mtry=17)
plot(cnt_alldayset_CVForest_result)
cnt_alldayset_CVForest_result$importance
varImpPlot(cnt_alldayset_CVForest_result,n.var = 17,color = "red",font = 2)
...
{
  all_cvrandomforest_results = data.frame(variable.num=integer(0), train.error=numeric(0), test.error=numeric(0))
  ...
  for (variable.num in 1:20)
  {
    ...
    #png(paste("all_Randomforset_variable#_",variable.num, ".png", sep=""))
   # plot(dayset$instant, cnt_alldayset$cnt, xlab="Time (Days)", ylab="Bikeshare Users")
   # points(dayset.train$instant, cntp_alldayset_CVForest_result.train, col="blue")
    #points(dayset.test$instant, cntp_alldayset_CVForest_result, col="red", pch=18,lwd=6)
    #mtext(paste("test.error:", test.error, sep = ""),side = 3,line = 1,adj = 1, col="red")
  #  mtext(paste("Num of Variables:",variable.num , sep = ""),side = 3,line = 2,adj = 1, col="blue")
   # dev.off()
    ...
    # save test and train error results for each trial
    all_cvrandomforest_results = rbind(all_cvrandomforest_results, cbind(variable.num, train.error, test.error))
  }
}
plot(x=all_cvrandomforest_results$variable.num,y=all_cvrandomforest_results$test.error,pch = 8,col='red')
points(x=all_cvrandomforest_results$variable.num,y=all_cvrandomforest_results$train.error,pch = 8,col='blue')
lines(x=all_cvrandomforest_results$variable.num,y=all_cvrandomforest_results$train.error,col='blue')
lines(x=all_cvrandomforest_results$variable.num,y=all_cvrandomforest_results$test.error,pch = 8,col='red')
mtext("Red: Testing error",adj = 1, line = 2,col = 'red')
mtext("Blue:Training error",adj = 1, line = 1,col = 'Blue')
